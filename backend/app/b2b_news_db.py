...
B2B_NEWS_SECTION_COLUMNS: dict[str, tuple[str, ...]] = {
    "news": ("Дата", "Новость", "Описание"),
    "launches": ("Дата", "Продукт", "Описание"),
}

_TITLE = "Новости и запуски"


def columns_for_section_gid(gid: str) -> tuple[str, ...]:
    columns = B2B_NEWS_SECTION_COLUMNS.get(gid)
    if columns is None:
...
def _save_section_snapshot(
    ...
    *,
    ...
    db.execute(
        text(
            """
            INSERT INTO b2b_news_snapshot (section_id, rows, changed_by)
...
def _replace_section_rows_from_snapshot(
    ...
    *,
    ...
    db.execute(
        text("DELETE FROM b2b_news_row WHERE section_id = :section_id"),
        {"section_id": section_id},
    )
    for index, item in enumerate(snapshot_rows):
        ...
        db.execute(
            text(
                """
                INSERT INTO b2b_news_row (section_id, sort_order, cells)
...
def load_b2b_news_from_db(
    ...
    *,
    ...
    if not sections:
        raise HTTPException(
            status_code=503,
            detail="Таблицы «Новости и запуски» не инициализированы.",
        )
    ...
    return ProductStatusB2BOut(
        title=_TITLE,
        sourceUrl=settings.b2b_news_sheet_public_url or None,
    ...
    *,
    ...
    for section_gid in processed_gids:
        ...
        for row_id in deleted_ids:
            ...
            db.execute(
                text(
                    "DELETE FROM b2b_news_row WHERE id = :row_id AND section_id = :section_id"
                ),
...
def delete_b2b_news_row(
    ...
    *,
    ...
    db.execute(
        text("DELETE FROM b2b_news_row WHERE id = :row_id AND section_id = :section_id"),
        {"row_id": row_id, "section_id": section_id},
    )
    db.commit()


def load_b2b_news_history(
    db: Session,
    *,
    gid: str,
    limit: int = 100,
