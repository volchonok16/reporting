...
| PPTX template | `backend/assets/b2b_product_status_template.pptx` | Локальный эталон для «Скачать презентацию» B2B (без Google). Переопределение: `B2B_PRODUCT_STATUS_PRESENTATION_TEMPLATE` |
...
Колонки (`gid=main`): «Активность», «Статус F2 2026», «Ответственный», «Влияние на базу, тыс», «Влияние на выручку, млн», «Влияние на gmc, млн», «Комментарий».  
Внизу таблицы — строка **Итого** (суммы по числовым колонкам). В заголовках — Excel-подобные фильтры по значениям.  
