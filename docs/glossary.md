...
| Roadmap приоритет | `extra_json.roadmap_priority` | Цвет колбаски на вкладке Roadmap: `red` — обязательно, `yellow` — средний приоритет, `green` — можно пропустить; `PATCH /api/tasks/{id}/roadmap-priority` |
