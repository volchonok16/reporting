...
Колонки (`gid=main`): «Активность», «Статус», «Ответственный», «Влияние на базу», «Влияние на выручку», «Влияние на gmc» (числовые), «Комментарий», «Результат» (только чтение — сумма трёх влияний; текстовые значения в числовых колонках в сумму не входят).  
