...
Сообщения пользователям (всем активным `org_user`, выбранным пользователям или по отделам) **без учёта ролей, вкладок и ограничений доступа**. Получать и читать могут все пользователи с учётной записью, включая Voice-only. Создаёт **только администратор** (`org_user.role=admin` или PAT). В UI — колокольчик; тип `popup` дополнительно сразу показывает модальное окно по центру экрана и звук.
...
| `GET/POST /api/notifications` | Inbox (по умолчанию только непрочитанные) и отправка уведомлений; получатели — все активные `org_user` при audience=`all`, независимо от ролей и доступа к вкладкам; звук при новых; тип `delivery=popup` — модальное окно по центру (опрос ~3 с). **Отправка только для администратора** |
...
| `a_number` | text | Номер A (отображение / сравнение / btree) |
...
Фиксированные ключи в `cells`: «Дата запуска», «Проект координация» (мультиселект из справочника офиса через `"; "`, либо свободный текст для офисов без seed), «Полное Описание проекта и статус», «Для презентации Описание проекта и статус», «Зачем и для чего делаем» (текст на слайде презентации), «ЗНИ» (несколько номеров через запятую с пробелом, напр. `123456, 789012`), «Идет в презентацию», «Обратить внимание», «Комментарий» (служебный, не в презентацию), «Приоритет» (число; 1 сверху), «Неактуальное» (да/нет; голубая подсветка строки). Легаси-ключи «Зачем и для чего делаем полное описание» / «…для презентации» при чтении сливаются в «Зачем и для чего делаем» (миграция `016_b2b_product_status_merge_why_columns.sql`).

## b2b_product_status_project — проекты координации по офисам

Справочник проектов для колонки «Проект координация». Seed: миграция `047_b2b_product_status_projects.sql` (SMS, VOICE, Перспективные продукты, M2M/IoT, Продуктовый маркетинг). Остальные офисы без записей — в UI остаётся свободный ввод.
...
|------|-----|----------|
| `id` | bigserial | PK |
| `gid` | varchar(32) | Стабильный ключ вкладки (`base`, `revenue`; архив `main`) |
...
Две независимые таблицы (строки не шарятся между вкладками):

| `gid` | Вкладка | Колонки |
|-------|---------|---------|
| `base` | Влияние по базе | «Активность», «Влияние на базу, тыс», «Влияние на gmc, млн», «Комментарий», «Статус F2 2026», «Ответственный» |
| `revenue` | Влияние по выручке | «Активность», «Влияние на выручку, млн», «Влияние на gmc, млн», «Комментарий», «Статус F2 2026», «Ответственный» |

В UI — колонка **№** (1, 2, 3…): по умолчанию сверху вниз; переключатель «Нумерация сверху/снизу» (сохраняется в localStorage). Новые строки добавляются в начало при нумерации сверху и в конец при нумерации снизу.  
Внизу таблицы — строка **Итого** (суммы по числовым колонкам вкладки). В заголовках — Excel-подобные фильтры по значениям.  
Экспорт: `GET/POST /api/revenue-activities/excel` — .xlsx (лист на вкладку), числовые колонки как числа, строка «Итого».  
