...
| Пирамида | `GET /api/org/org-chart?department_id=` — без `department_id`: директор и отделы; отдел вкладывается в ветку вышестоящего, если его руководитель — head другого отдела; для одного отдела — дерево по составу |
