...
Клик по карточке метрики фильтрует таблицу (`metric=launching_soon|launched|completed|errors`); повторный клик снимает фильтр. Период «Дата начала»–«Дата конца» **не** ограничивает список и «Всего задач» / «Скоро запуск» / «С ошибками» по `start_date` или дате создания. Период учитывается только в метриках **«Запущено»** (переход в Pilot) и **«Завершенные»** (переход в Closed).
