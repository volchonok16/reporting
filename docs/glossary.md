...
| Пирамида | `GET /api/org/org-chart?department_id=` — без `department_id`: директор и отделы (каждый отдел — отдельная ветка; подчинение руководителей отделов друг другу не вкладывает отделы); для одного отдела — дерево по составу |
