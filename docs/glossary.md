...
| Roadmap приоритет | `extra_json.roadmap_priority` | Локальное поле (не в TFS): цвет колбаски на Roadmap — `red` / `yellow` / `green`; `PATCH /api/tasks/{id}/roadmap-priority`; при синхронизации TFS сохраняется в БД |
