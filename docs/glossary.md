...
**Фильтр области (дашборд):** только доска **Digital** (`digital_streams_b2b`). В UI — «Область»; query-параметр `tag_group` (можно несколько); на остальных досках и в «Все доски» игнорируется. Группы задаются в `backend/app/tag_filters.py`:

| Ключ API | Подпись в UI | Корневой тег TFS | Подразделы (префикс) |
|----------|--------------|------------------|----------------------|
| `newlk` | ЛК b2b | `LK_B2B` | `lk_*` (напр. `lk_serv`) |
| `site` | Сайт | `site_b2b` | `site_*` |
...
Клик по карточке метрики фильтрует таблицу (`metric=in_progress|launching_soon|launched|completed|errors`); повторный клик снимает фильтр. Период «Дата начала»–«Дата конца» по `start_date` / `created_at` применяется к **«Всего задач»**, общему списку и **«Ошибки»**. **«В работе»**, **«Скоро запуск»**, **«Запущено»**, **«Завершенные»** и **«Ошибки»** — без фильтра по дате начала. Закрытые ЗНИ в таблице видны при фильтрах **«Запущено»** (BE: `Closed`) и **«Завершенные»**. Закрытые ошибки не попадают в счётчик, фильтр и колонку «Ошибки» CSV-выгрузки.
