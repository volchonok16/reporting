...
Клик по карточке метрики фильтрует таблицу (`metric=launching_soon|launched|completed|errors`); повторный клик снимает фильтр. Числа на карточках и строки таблицы считаются в выбранном периоде «Дата начала»–«Дата конца» (для «Завершенные» — по дате перехода в Closed, без фильтра по дате начала).
