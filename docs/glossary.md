...
| Related → CRM / Bercut | `extra_json.linked_environments` | Массив связанных ЗНИ в окружениях **CRM** (`Tele2\Продукты`) и **Bercut** (`BE-T2\BE Analytics`): `key`, `label`, `zni_id`, `status`, `board_column`, `url`. Заполняется при синхронизации доски **Digital** (WIQL Related между областями). В дашборде ЗНИ — в раскрывающейся панели строки (только доска Digital) |
