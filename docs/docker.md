...
Вкладка **Voice** — стек в каталоге `voice/` (FastAPI + Next.js). Сервисы: `voice-api` и `voice-web`. Данные сессий/uploads: `./voice/data` (права на запись для UID 10001). **Мастер-файл** — в PostgreSQL reporting (`DATABASE_URL` у `voice-api`, миграция `050_voice_master.sql`). Код API в prod/offline — из образа. В проде Voice открывается **в том же приложении reporting**: `voice-web` с `VOICE_BASE_PATH=/voice`; frontend-nginx проксирует `/voice/` → `voice-web`, `/voice/assets/` → `voice-web:/assets/` (vinext отдаёт статику без basePath), `/assets/_vinext_fonts/` → voice-web, `/voice-api/` → `voice-api`. Хостовый nginx (`snippets/voice-proxy.conf`) шлёт `/voice*` на `:5173` (не напрямую на `:3100`). Маркер: `GET /voice/reporting-voice.txt` → `voice-ok` (если вернулся HTML reporting — старый образ frontend). Прямой порт: `http://127.0.0.1:3100/voice/`. Вкладка доступна всем; при флаге **Voice сервисы** (`org_user.voice_only`) — только Voice. SSO: `VOICE_SSO_SECRET`. Offline-bundle: `reporting/voice-api:offline`, `reporting/voice-web:offline`. Миграция `046_employee_hide_from_pyramid.sql` — «Не отображать в пирамиде».
