...
Вкладка **Voice** — стек в каталоге `voice/` (FastAPI + Next.js). Сервисы: `voice-api` (`localhost:8100`) и `voice-web` (`localhost:3100`). Данные: `./voice/data`. Frontend открывает iframe с SSO (`VITE_VOICE_APP_URL`); общий секрет `VOICE_SSO_SECRET` должен совпадать у `backend` и `voice-api`.
