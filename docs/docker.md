...
**Презентация B2B:** шаблон — локальный файл `backend/assets/Status.pptx` (копируется в образ). Приоритет: `B2B_PRODUCT_STATUS_PRESENTATION_TEMPLATE` → bundled assets → только если файла нет, попытка export из `B2B_PRODUCT_STATUS_PRESENTATION_REFERENCE_URL`. На контуре без Google локальный PPTX обязателен (уже в репозитории). Ссылка REFERENCE_URL остаётся для кнопки «Эталон в Google Slides» в UI.
