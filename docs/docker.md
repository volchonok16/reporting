...
Переменные `B2B_PRODUCT_STATUS_*`, `B2B_NEWS_*`, `GOOGLE_SHEETS_API_KEY`, `GOOGLE_SHEETS_WORKBOOK_CACHE_TTL_SECONDS` (по умолчанию 300 с — in-memory кеш листов на backend) и `GOOGLE_SHEETS_SERVICE_ACCOUNT_JSON` пробрасываются в контейнер `backend` через `docker-compose.yml`. `ZNI_ACTUAL_PERIOD_EDITABLE_STATES` (по умолчанию `UAT,Pilot,Closed`) задаёт статусы ЗНИ, в которых можно править «Фактическая дата месяц/квартал».
...
# или точечно:
docker rm -f reporting-voice-api reporting-voice-web reporting-backend
docker-compose -f docker-compose.yml -f docker-compose.prod.yml -f docker-compose.offline.yml up -d --no-build backend voice-web
```

Offline-образы app-сервисов flatten’ятся в `offline-bundle.sh` (без attestation), иначе docker-compose 1.29 падает с `ContainerConfig`.
...
Статус продукта B2B в PostgreSQL — `db/migrations/013_b2b_product_status.sql`: таблицы `b2b_product_status_office`, `b2b_product_status_row`, `b2b_product_status_history` и seed вкладок офисов; `db/migrations/014_b2b_product_status_snapshots.sql` — снимки версий для отката; `db/migrations/015_b2b_news.sql` — «Новости и запуски» в БД; `db/migrations/016_b2b_product_status_merge_why_columns.sql` — объединение двух столбцов «Зачем» в один; `db/migrations/030_b2b_product_status_offices_analytics_projects.sql` — вкладка «Офис: Проекты (Саша и Ваня)» (и исторически «Офис: Аналитики»); `db/migrations/044_b2b_product_status_analytics_split.sql` — вместо «Аналитики»: «Офис: Аналитики: планирование» и «Офис: Аналитики: бизнес-анализа»; `db/migrations/047_b2b_product_status_projects.sql` — справочник `b2b_product_status_project` (проекты координации по офисам); `db/migrations/062_b2b_product_status_office_editing_lock.sql` — блокировка редактирования по офисам (`b2b_product_status_office.editing_locked`); `db/migrations/031_revenue_activities.sql` — «Активности по выручкам» (`revenue_activity_*`); `032`–`035` — колонки/единицы (идемпотентно); `036_revenue_activities_base_revenue_sections.sql` — вкладки «Влияние по базе» (`base`) и «Влияние по выручке» (`revenue`), копирование строк из `main`, деактивация `main`; `037_revenue_activities_margin_column.sql` — исторически колонка «Маржа»; `038_revenue_activities_drop_margin.sql` — «Маржа» убрана из UI/колонок; `039_planning.sql` — планирование ресурсов (`planning_project`, `planning_allocation`, `production_calendar_day` и др.); `040_planning_project_executors.sql` — исполнители проекта (`planning_project_executor`); `041_planning_project_status.sql` — статус проекта (`planning_project.status`); `042_planning_customer_department.sql` — справочник департаментов заказчика (`planning_customer_department`); `043_zni_boards.sql` — конфиг досок ЗНИ (`zni_board`: alias, area, теги, TFS GUID); `048_zni_external_data.sql` — внешние поля ЗНИ (`zni_external_data`); `059_zni_external_data_comment.sql` — колонка «Комментарий» в доп. полях ЗНИ; `063_zni_category.sql` — справочник категорий ЗНИ (`zni_category`) и `zni_external_data.category_id`; `064_zni_drop_desired_quarter.sql` — удаление колонки `desired_quarter`; `065_task_iteration_path_backfill.sql` — заполнение `task.iteration_path` из `extra_json`; `db/migrations/060_app_pages.sql` — справочник вкладок `app_page` и доступ `org_user_page_access` для «других пользователей»; `061_app_notification_delivery.sql` — тип доставки уведомления (`app_notification.delivery`: `inbox` / `popup`); `049_app_notifications.sql` — уведомления (`app_notification`, `app_notification_recipient`); `045_org_user_voice_only.sql` — флаг `org_user.voice_only` (доступ только к вкладке Voice); `054_org_user_voice_admin.sql` — флаг `org_user.voice_admin` (**Администратор Voice**: очистка журнала и мастер-файла); `000_schema_migration.sql` — журнал `schema_migration`. Миграции при старте backend (`ensure_startup_schema`) применяются **один раз** и отмечаются в `schema_migration`. Data-миграции `032`–`035`, `037`–`038` **не перезаписывают** `revenue_activity_row.cells` (только COMMENT/SELECT 1); старые ключи читаются aliases в backend.

Вкладка **Voice** — стек в каталоге `voice/` (FastAPI + Next.js). **Voice API** встроен в **reporting backend** (`/voice-api`, код `voice/backend` в образе как `carousel/`); отдельного контейнера `voice-api` нет. UI — сервис `voice-web`. **Метаданные uploads/jobs** — PostgreSQL reporting (`051_voice_registry.sql`: `voice_uploads`, `voice_jobs`). **Auth** — только reporting SSO (без таблиц auth в Voice). **Мастер-файл** — там же (`050_voice_master.sql`, `master_*`; `053_voice_master_signature_hash.sql` — hash PK/индексы для длинных связок B; `056_voice_master_numeric_ids.sql` — PK/`import_id`/`item_id`/`record_id` как `BIGINT IDENTITY`; `057_voice_master_rollback_numeric_a_indexes.sql` — откат `a_number_key`/`055`, снова btree по TEXT `a_number`; `058_voice_master_prefix_index.sql` — partial index по `source_prefix` для фильтров). `DATABASE_URL` обязателен у backend. В проде Voice открывается **в том же приложении reporting**: `voice-web` с `VOICE_BASE_PATH=/voice`; frontend-nginx проксирует `/voice/` → `voice-web`, `/voice/assets/` → `voice-web:/assets/` (vinext отдаёт статику без basePath), `/assets/_vinext_fonts/` → voice-web, `/voice-api/` → `backend:8000/voice-api/`. Хостовый nginx (`snippets/voice-proxy.conf`) шлёт `/voice*` на `:5173` (не напрямую на `:3100`). Маркер: `GET /voice/reporting-voice.txt` → `voice-ok` (если вернулся HTML reporting — старый образ frontend). Прямой порт UI: `http://127.0.0.1:3100/voice/`. Health Voice API: `GET /voice-api/api/health`. Prod: `BACKEND_WORKERS=1` (Voice job executor). Файлы uploads — `voice/data` → `/data` в backend. Вкладка доступна всем; при флаге **Voice сервисы** (`org_user.voice_only`) — только Voice. SSO: `VOICE_SSO_SECRET`. Offline-bundle: `reporting/backend:offline` (включая Voice API), `reporting/voice-web:offline`. Миграция `046_employee_hide_from_pyramid.sql` — «Не отображать в пирамиде».
