...
Вкладка **Voice** — стек в каталоге `voice/` (FastAPI + Next.js). Сервисы: `voice-api` (`localhost:8100`) и `voice-web` (`localhost:3100`). Данные: `./voice/data` (нужны права на запись для UID 10001). Код API в prod/offline — из образа; bind `./voice/backend` только в `docker-compose.dev.yml`. Frontend открывает iframe с SSO (`VITE_VOICE_APP_URL` или `hostname:3100`); общий секрет `VOICE_SSO_SECRET` должен совпадать у `backend` и `voice-api`. Offline-bundle включает образы `reporting/voice-api:offline` и `reporting/voice-web:offline`.
