...
Вкладка **Voice** — отдельный стек в каталоге `voice/` (FastAPI + Next.js). Сервисы Docker Compose: `voice-api` (`localhost:8100`) и `voice-web` (`localhost:3100`). Данные Voice: bind-mount `./voice/data`. Frontend открывает Voice в iframe (`VITE_VOICE_APP_URL`, по умолчанию `http://localhost:3100`).
