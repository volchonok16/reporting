...
Вкладка **Voice** — стек в каталоге `voice/` (FastAPI + Next.js). Сервисы: `voice-api` и `voice-web`. Данные: `./voice/data` (права на запись для UID 10001). Код API в prod/offline — из образа. В проде Voice открывается **в том же приложении reporting**: frontend-nginx проксирует `/voice/` → `voice-web` и `/voice-api/` → `voice-api` (хостовый nginx достаточно слать всё на `:5173`). Вкладка видна только при флаге **Voice сервисы** (`org_user.voice_only`); у таких пользователей остальных вкладок нет. SSO: `VOICE_SSO_SECRET`. Offline-bundle включает `reporting/voice-api:offline` и `reporting/voice-web:offline`. Миграция `046_employee_hide_from_pyramid.sql` — флаг «Не отображать в пирамиде».
