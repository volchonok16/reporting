...
Вкладка **Voice** — стек в каталоге `voice/` (FastAPI + Next.js). Сервисы: `voice-api` (`localhost:8100`) и `voice-web` (`localhost:3100`). Данные: `./voice/data` (нужны права на запись для UID 10001). Код API в prod/offline — из образа; bind `./voice/backend` только в `docker-compose.dev.yml`. Frontend открывает iframe на same-origin `/voice/` (nginx snippet `voice-proxy.conf`); API Voice — `/voice-api/`. Тема iframe следует за reporting. Секрет `VOICE_SSO_SECRET` должен совпадать у `backend` и `voice-api`. Offline-bundle включает образы `reporting/voice-api:offline` и `reporting/voice-web:offline`. Миграция `046_employee_hide_from_pyramid.sql` — флаг «Не отображать в пирамиде».
