...
def test_master_lock_is_exclusive_and_persists(tmp_path) -> None:
    ...
    lock = MasterLockService(registry, auth)

    owner_login = auth.login_with_reporting_sso(
        email=config.auth_bootstrap_email,
        is_admin=True,
    )
    owner_token = owner_login["token"]
    owner = auth.authenticate(owner_token)
    other_login = auth.login_with_reporting_sso(
    ...
    restarted_service = MasterLockService(registry, auth)
    ...
    second_owner_login = auth.login_with_reporting_sso(
        email=config.auth_bootstrap_email,
        is_admin=True,
    )
    ...
    assert acquired["ownedByCurrentSession"] is True
    restarted_service.release(second_owner, second_owner_token)
    ...
    with registry.connect() as connection:
        connection.execute(
            "UPDATE master_edit_lock SET owner_session_expires_at = 0 WHERE id = 1"
        )
    force_login = auth.login_with_reporting_sso(
        email=config.auth_bootstrap_email,
        is_admin=True,
    )
    ...
    other_login_again = auth.login_with_reporting_sso(
        email="editor@example.test",
        is_admin=False,
    )
